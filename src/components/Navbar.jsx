  ...
  return (
    <nav ref={navRef} style={{ opacity: 0 }} className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      ...
      <div className="max-w-7xl mx-auto px-6 py-4">
        ...
        <div className="flex items-center justify-between mb-3">
          <a href="#home" className="flex items-center gap-3 group">
            <div className="w-10 h-10 rounded-2xl border-2 border-[var(--mahenti-accent)] flex items-center justify-center group-hover:glow-accent transition-all duration-300">
              <span className="text-[var(--mahenti-accent)] font-black text-xl font-montserrat">S</span>
            </div>
            <div>
              <div className="text-white font-bold text-base tracking-widest leading-none font-montserrat">STANDARD GROUP</div>
              <div className="text-[var(--mahenti-accent)] text-[10px] tracking-[0.3em] uppercase font-montserrat font-bold not-italic">We Set Standards</div>
            </div>
          </a>
        ...
        {/* Bottom row: nav tabs */}
        <div ref={linksRef} className="hidden md:flex items-center justify-center gap-8 border-t border-zinc-800/50 pt-3">
          {navLinks.map((link) => (
